...
class Protocol(object):
    ...
    def __init__(self):
        ...
        self._pkt_state = Protocol.PKT_IDLE
        self._pkt_dlen = 0
    ...
    def process(self, buf):
        ret = Protocol.LIDAR_IN_PROCESS
        for b in buf:
            if self._pkt_state == Protocol.PKT_IDLE:
                if b == 0xAA:
                    self._pkt_pos = 0
                    self._pkt_buf[self._pkt_pos] = b
                    self._pkt_pos += 1
                    self._pkt_state = Protocol.PKT_SYNC

            elif self._pkt_state == Protocol.PKT_SYNC:
                if b == 0x55:
                    self._pkt_buf[self._pkt_pos] = b
                    self._pkt_pos += 1
                    self._pkt_state = Protocol.PKT_HEADER
                else:
                    self._pkt_state = Protocol.PKT_IDLE

            elif self._pkt_state == Protocol.PKT_HEADER:
                if self._pkt_pos < 10:
                    self._pkt_buf[self._pkt_pos] = b
                    self._pkt_pos += 1
                if self._pkt_pos == 10:
                    ...
                    self._dlen *= 2
                    self._pkt_state = Protocol.PKT_DATA

            elif self._pkt_state == Protocol.PKT_DATA:
                ...
                if self._pkt_pos == (10 + self._dlen):
                    if self.calc_checksum(self._pkt_buf, self._pkt_pos) == 0:
                        ...
                        ret = Protocol.LIDAR_SCAN_COMPLETED if state else Protocol.LIDAR_PACKET_DECODED
                    self._pkt_state = Protocol.PKT_IDLE
        return ret
...
class LidarX2(object):
        ...
        while not sig_kill.is_set():
            try:
                ...
                if avail > 0:
                    ...
                    if state == Protocol.LIDAR_SCAN_COMPLETED and self._screen != None:
                        frame = self._protocol.get_scans()
                        if frame is not None:
                            self._protocol.draw(self._screen, frame)
                else:
                    time.sleep(0.005)
    ...
    def cmd_callback(self, cmd, data):
        if cmd == 1:
            frame = {'ts': 0, 'scan_num': 0, 'scans': [0] * self._protocol._max_scans}
            frame['ts'], frame['scan_num'] = struct.unpack('<LH', data[0:6])
            for i in range(self._protocol._max_scans):
